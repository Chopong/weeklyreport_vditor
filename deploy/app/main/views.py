...
from ..utils import permission_required, is_allowed_img, is_allowed_file, clean_html
...
def upload():
    ...
    if img and is_allowed_img(img.filename):
        filename_ext = datetime.now().strftime("%Y%m%d_%H%M%S_")
        filename = secure_filename(filename_ext+img.filename)
        img.save(os.path.join(current_app.config['UPLOAD_FOLDER'], filename))
        img_url = request.url_root + \
            current_app.config['IMAGE_UPLOAD_DIR'] + filename
        # res = Response(img_url)
        res = { "msg": "",
                "code": 0,
                "data": {
                    "errFiles": [],
                    "succMap":{filename: img_url}
                    }
                }

        current_app.logger.info('{} uploaded image'.format(current_user.email))
    else:
        # res = Response(_("Failed Uploading"))
        res = { "msg": "Can not find imgs",
                "code": 1,
                "data": {
                    "errFiles": [],
                    "succMap":{}
        ...
        current_app.logger.error('{} failed uploading image'.format(current_user.email))
...
@main.route("/cpuploadfile/", methods=["POST"])
@permission_required(Permission.WRITE_REPORT)
def cpuploadfile():
    ...
    if file:
        filename_ext = datetime.now().strftime("%Y%m%d_%H%M%S_")
        filename = secure_filename(filename_ext+file.filename)
        file.save(os.path.join(current_app.config['UPLOAD_FOLDER'], filename))
        file_url = request.url_root + current_app.config['IMAGE_UPLOAD_DIR'] + filename
        res = { "msg": "",
                "code": 0,
                "data": {
                    "errFiles": [],
                    "succMap":{filename: file_url}
                    }
                }
        current_app.logger.info('{} uploaded image'.format(current_user.email))
    else:
        ...
        current_app.logger.error('{} failed uploading files'.format(current_user.email))
    return res
